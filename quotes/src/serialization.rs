...
macro_rules! read_num {
    ($reader:expr, $ty:ty) => {{
        ...
        $reader
            ...
            .map_err(|_| super::Error::InvalidData(INVALID_FORMAT_MSG.to_string()))?;
...
impl TryFrom<Vec<u8>> for StockQuote {
    ...
    fn try_from(value: Vec<u8>) -> Result<Self, Self::Error> {
        ...
        reader
            .read_exact(&mut ticker)
            .map_err(|e| {
                eprintln!("{}", e);
                super::Error::InvalidData(INVALID_FORMAT_MSG.to_string())
            })?;
        let ticker = String::from_utf8(ticker).map_err(|_| {
            super::Error::InvalidData("Описание должно быть в формате UTF-8".to_string())
        })?;
...
fn max_ticker_len_error(ticker_len: usize) -> super::Error {
    super::Error::InvalidData(format!(
...
mod tests {
    ...
    #[test]
    #[should_panic(
        expected = "Максимальный размер размер тикера: 4, получено: 5"
    )]
